    ...
    func add(todo: Todo, completion: @escaping (Error?) -> Void) {
        ...
        connection.connect() { error in
            if let error = error {
                ...
                let query = Insert(into: todos, valueTuples: (todos.title, todo.title))
    ...
    func update(todo: Todo, completion: @escaping (Error?) -> Void) {
        ...
        connection.connect() { error in
            if let error = error {
                ...
                let query = Update(todos, set: [(todos.title, todo.title)], where: todos.id == todo.id)
    ...
    func delete(id: String, completion: @escaping (Error?) -> Void) {
        ...
        connection.connect() { error in
            if let error = error {
                ...
                let query = Delete(from: todos, where: todos.id == id)
